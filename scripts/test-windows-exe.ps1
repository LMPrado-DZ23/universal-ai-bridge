...
function Run-Exe([string]$File,[string[]]$Arguments,[string]$Phase) {
  ...
  if($p.ExitCode -ne 0){throw "EXE failed: $Phase (exit $($p.ExitCode))."}
}
$completed=@()
try {
  ...
  Get-ChildItem $evidence -Filter 'exe-*.log' | ForEach-Object {
    ...
    if($completed.Count -lt 4){Get-Content $_.FullName -Tail 20 | Write-Output}
  }
