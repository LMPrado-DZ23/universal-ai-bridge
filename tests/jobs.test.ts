...
import { tokenize } from "../src/exec.js";

const wait = (ms: number) => new Promise((r) => setTimeout(r, ms));
const node = process.execPath; // caminho absoluto do node (exe real)
...
describe("JobManager (execução estruturada, shell:false)", () => {
  it("roda um job e captura stdout + exit code", async () => {
    const dir = mkdtempSync(join(tmpdir(), "uab-job-"));
    const m = new JobManager();
    const id = m.start(node, ["-e", "process.stdout.write('ola-job')"], dir);
    await waitDone(m, id);
    expect(m.output(id).stdout).toContain("ola-job");
  ...
  it("cancela um job de longa duração (mata a árvore)", async () => {
    const dir = mkdtempSync(join(tmpdir(), "uab-job-"));
    const m = new JobManager();
    const id = m.start(node, ["-e", "setInterval(()=>{},1000)"], dir);
    await wait(300);
  ...
  it("saída incremental avança com o cursor", async () => {
    const dir = mkdtempSync(join(tmpdir(), "uab-job-"));
    const m = new JobManager();
    const id = m.start(node, ["-e", "process.stdout.write('abcdef')"], dir);
    await waitDone(m, id);
  ...
  it("destroy() encerra e remove do registro (cleanup de sessão)", async () => {
    const dir = mkdtempSync(join(tmpdir(), "uab-job-"));
    const m = new JobManager();
    const id = m.start(node, ["-e", "setInterval(()=>{},1000)"], dir);
    await wait(200);
